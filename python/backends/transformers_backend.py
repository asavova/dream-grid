...
class TransformersAnalysisBackend:
    name = "transformers"
    ...
    def __init__(
        ...
        self.model_client = model_client or TransformersTextGenerator(model_name=model_name)
        self.model_version = model_version

    def analyze(self, dream_text: str) -> AnalysisResult:
        prompt = self._build_analysis_prompt(dream_text)
        model_output = self.model_client.generate_text(prompt)
        payload = self._parse_json_object(model_output)

        return AnalysisResult(
            summary=self._normalize_text(payload.get("summary")),
            detectedSymbols=self._normalize_list(payload.get("detectedSymbols")),
            detectedThemes=self._normalize_list(payload.get("detectedThemes")),
            confidenceScore=self._normalize_confidence(payload.get("confidenceScore"), payload),
            modelVersion=self.model_version,
        )

    def answer_question(self, dream_text: str, analysis_result: str, question: str) -> str:
        prompt = self._build_question_prompt(dream_text, analysis_result, question)
        answer = self.model_client.generate_text(prompt)
        return self._normalize_answer(answer)

    def _build_analysis_prompt(self, dream_text: str) -> str:
        return f"""
...
""".strip()
    ...
    def _build_question_prompt(self, dream_text: str, analysis_result: str, question: str) -> str:
        return f"""
...
""".strip()
    ...
    def _parse_json_object(self, model_output: str) -> dict:
        try:
    ...
    def _normalize_confidence(self, value, payload: dict) -> float:
        ...
        if isinstance(payload.get("summary"), str) and payload.get("summary").strip():
            score += 0.25
        if self._normalize_list(payload.get("detectedSymbols")):
            score += 0.2
        if self._normalize_list(payload.get("detectedThemes")):
            score += 0.1
